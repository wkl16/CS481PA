...
void MakeTransactions(struct Bank* Bank) {
  ...
  for (i = 0; i < 100; i++) {
    ...
    sem_wait(&Bank->mutex);  // Enter critical section
    ...
    sem_post(&Bank->mutex);  // Exit critical section
  }
}

int main(int argc, char** argv) {
  key_t key = ftok("proc_sem.c", 65);
